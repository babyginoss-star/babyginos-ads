...
export const CONFIG = {
  ...
  CPA_ATENCION:    70000,  // semáforo naranja: $56k-$70k / rojo: >$70k
                           // cualquier CPA > CPA_PROMETEDOR con gasto → Atención (sin techo)

  // Mínimo de compras en ventana corta para clasificar con gasto bajo
...
export function evaluateAd(ad, snapshots, ctx = {}) {
  ...
  // tieneComprasSuficientes: >= 2 compras promedio en últimos 7 días
  ...
  const costoAtencion   = avgCPR != null && avgCPR > CONFIG.CPA_PROMETEDOR; // > 55k, sin techo
  const cpaRojo         = avgCPR != null && avgCPR > CONFIG.CPA_ATENCION;   // > 70k
  ...
  // 3a. Pausar: gasto alto + CPA > $70k → pausarlo ya (aunque tenga 1 sola compra)
  if (gastoAlto && cpaRojo) {
    return { funnel, status: "pausar",
      mensaje: "Pausar — CPA rojo con gasto alto. Pausalo ahora." };
  }

  // 3b. Atención: clasificable + CPA > $55k → decidí si pausás
  ...
  // 4. El que no suma: gasto bajo + freq baja + CPM alto + sin datos
